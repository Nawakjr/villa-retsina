...
export default function ContactSection({ contact }: { contact: Content["contact"] }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        setError(data.error || contact.errorGeneric);
        setStatus("error");
        return;
      }
      setStatus("ok");
      ...
      setError(contact.errorNetwork);
      setStatus("error");
    }
  }

  const ph = contact.placeholders;
  ...
  return (
    <section id="contact" className="scroll-mt-20 bg-anthracite text-sable">
      <div className="mx-auto grid max-w-6xl gap-12 px-6 py-20 sm:py-28 md:grid-cols-[0.85fr_1.15fr] md:gap-16">
        {/* Colonne info */}
        <div>
          <h2 className="font-serif text-4xl text-sable sm:text-5xl">{contact.heading}</h2>
        ...
        <form onSubmit={onSubmit} className="grid gap-4 sm:grid-cols-2">
          ...
          <Field name="name" placeholder={ph.name} required autoComplete="name" />
          <Field name="email" type="email" placeholder={ph.email} required autoComplete="email" />
          <Field name="phone" type="tel" placeholder={ph.phone} autoComplete="tel" />
          <Field name="dates" placeholder={ph.dates} />
          <Field name="guests" placeholder={ph.guests} className="sm:col-span-2" />
          ...
          <div className="sm:col-span-2 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            {status === "ok" && <p className="text-sm text-olive">{contact.success}</p>}
            {status === "error" && <p className="text-sm text-[#e0a17f]">{error}</p>}
          </div>
        </form>
      </div>
    </section>
